...
const saleItems = products
  ...
  .map((item, index) => ({
    ...item,
    salePrice: Math.max(19, Math.round(item.price * (index % 2 === 0 ? 0.7 : 0.6))),
  }));
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {saleItems.map((item) => (
            <Link key={item.id} href={`/product/${item.slug}`} className="group border border-neutral-300 bg-white p-3">
              ...
              <p className="mt-1 text-sm text-neutral-500 line-through">${item.price.toFixed(2)}</p>
              <p className="text-base text-[#111111]">${item.salePrice.toFixed(2)}</p>
            </Link>
          ))}
        </div>
      </section>
